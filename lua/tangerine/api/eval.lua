...
local function get_lines(start, _end)
  _G.assert((nil ~= _end), "Missing argument end on tangerine/api/eval.fnl:26")
  _G.assert((nil ~= start), "Missing argument start on tangerine/api/eval.fnl:26")
...
local function tbl_merge(tbl1, tbl2)
  _G.assert((nil ~= tbl2), "Missing argument tbl2 on tangerine/api/eval.fnl:37")
  _G.assert((nil ~= tbl1), "Missing argument tbl1 on tangerine/api/eval.fnl:37")
  return vim.tbl_extend("keep", tbl1, tbl2)
end
eval.string = function(str, _3fopts)
  _G.assert((nil ~= str), "Missing argument str on tangerine/api/eval.fnl:45")
...
eval.file = function(path, _3fopts)
  _G.assert((nil ~= path), "Missing argument path on tangerine/api/eval.fnl:61")
  local opts = (_3fopts or {})
...
eval.buffer = function(start, _end, _3fopts)
  _G.assert((nil ~= _end), "Missing argument end on tangerine/api/eval.fnl:72")
  _G.assert((nil ~= start), "Missing argument start on tangerine/api/eval.fnl:72")
  local opts = (_3fopts or {})
...
eval.peek = function(start, _end, _3fopts)
  _G.assert((nil ~= _end), "Missing argument end on tangerine/api/eval.fnl:90")
  _G.assert((nil ~= start), "Missing argument start on tangerine/api/eval.fnl:90")
  local opts = (_3fopts or {})
  local fennel0 = fennel.load()
  local start0 = (start - 1)
  local lines = get_lines(start0, _end)
